...
fn main() {
    ...
    fn ping(core: &mut Core, mut swarm: Swarm, peer: PeerInfo) {
        ...
        println!("ping result {:#?}", String::from_utf8(result.to_vec()).unwrap());
    }
    ...
    if arg == Some("listen".to_owned()) {
        ...
        match result {
            Either::A((swarm, _)) => println!("Swarm ended: {:#?}", swarm),
            Either::B((_, swarm)) => println!("Swarm still running: {:#?}", swarm),
        }
    }
}
